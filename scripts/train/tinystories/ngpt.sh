...
OUTPUT_DIR="${OUTPUT_DIR:-${REPO_ROOT}/experiments/tinystories/${RUN_NAME}}"   # ckpt.pt/stat/args/finished + logs
...
cd "${REPO_ROOT}"
# nGPT: no weight decay, no warmup, base_scale=1/sqrt(n_embd) (train.py derives these from use_nGPT=1)
torchrun --nnodes=1 --nproc_per_node="${DEVICES}" --rdzv_backend=c10d --rdzv_endpoint=localhost:0 train.py \
    ...
    --compile="${COMPILE}" \
    --dataset=tinystories --data_dir="${DATA_DIR}" --out_dir="${OUTPUT_DIR}" \
